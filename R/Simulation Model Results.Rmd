...
fit_all_models <- function(dataset_list, batch_process_range,
  ...
  for (batch in (batch_process_range)) {
    ...
    if (NNet == 1) {
      ...
      simulation_results_list[[batch]]$NN1_MSE <- NNet_fit_stats(pooled_panel, timeSlices, 1, "mse", batch_size = 32, 20)
      simulation_results_list[[batch]]$NN1_MAE <- NNet_fit_stats(pooled_panel, timeSlices, 1, "mae", batch_size = 32, 20)
    
      simulation_results_list[[batch]]$NN2_MSE <- NNet_fit_stats(pooled_panel, timeSlices, 2, "mse", batch_size = 32, 20)
      simulation_results_list[[batch]]$NN2_MAE <- NNet_fit_stats(pooled_panel, timeSlices, 2, "mse", batch_size = 32, 20)
    
      simulation_results_list[[batch]]$NN3_MSE <- NNet_fit_stats(pooled_panel, timeSlices, 3, "mse", batch_size = 32, 20)
      simulation_results_list[[batch]]$NN3_MAE <- NNet_fit_stats(pooled_panel, timeSlices, 3, "mse", batch_size = 32, 20)
    
      simulation_results_list[[batch]]$NN4_MSE <- NNet_fit_stats(pooled_panel, timeSlices, 4, "mse", batch_size = 32, 20)
      simulation_results_list[[batch]]$NN4_MAE <- NNet_fit_stats(pooled_panel, timeSlices, 4, "mse", batch_size = 32, 20)
    
      simulation_results_list[[batch]]$NN5_MSE <- NNet_fit_stats(pooled_panel, timeSlices, 5, "mse", batch_size = 32, 20)
      simulation_results_list[[batch]]$NN5_MAE <- NNet_fit_stats(pooled_panel, timeSlices, 5, "mse", batch_size = 32, 20)
    }
  }
  simulation_results_list
}

g1_A1 <- fit_all_models(g1_A1_panel, batch_process_range, LM = 1, ELN = 1, RF = 0, NNet = 0)
g1_A2 <- fit_all_models(g1_A2_panel, batch_process_range, LM = 1, ELN = 1, RF = 0, NNet = 0)
g1_A3 <- fit_all_models(g1_A3_panel, batch_process_range, LM = 1, ELN = 1, RF = 0, NNet = 0)

g2_A1 <- fit_all_models(g2_A1_panel, batch_process_range, LM = 1, ELN = 1, RF = 0, NNet = 0)
g2_A2 <- fit_all_models(g2_A2_panel, batch_process_range, LM = 1, ELN = 1, RF = 0, NNet = 0)
g2_A3 <- fit_all_models(g2_A3_panel, batch_process_range, LM = 1, ELN = 1, RF = 0, NNet = 0)

g3_A1 <- fit_all_models(g3_A1_panel, batch_process_range, LM = 1, ELN = 1, RF = 0, NNet = 0)
g3_A2 <- fit_all_models(g3_A2_panel, batch_process_range, LM = 1, ELN = 1, RF = 0, NNet = 0)
g3_A3 <- fit_all_models(g3_A3_panel, batch_process_range, LM = 1, ELN = 1, RF = 0, NNet = 0)
```
...
combined_importance <- data.frame(
  rbind(data.frame(cbind(g1_A2[[1]]$LM_MSE[[1]]$variable_importance, model = "LM_MSE")),
        data.frame(cbind(g1_A2[[1]]$LM_MSE[[1]]$variable_importance, model = "LM_MAE")),
        data.frame(cbind(g1_A2[[1]]$LM_MSE[[1]]$variable_importance, model = "ELN_MAE")),
        data.frame(cbind(g1_A2[[1]]$LM_MSE[[1]]$variable_importance, model = "ELN_MSE"))
)) %>%
...
ggplot(combined_importance %>% filter(importance > 0.01) %>% mutate(variable = fct_reorder(variable, importance))) +
...
# Build a matrix of forecasts with columns corresponding to time index, rows corresponding to different models

cbind(matrix(g1_A2[[1]]$LM_MSE[[1]]$forecasts),  matrix(g1_A2[[1]]$LM_MAE[[1]]$forecasts))

test <- g1_A2_panel[[1]]$panel %>% filter(time %in% timeSlices[[1]]$test)

diebold_test <- MDM.test(realized = test$rt, 
                         evaluated = t(cbind(matrix(g1_A2[[1]]$LM_MSE[[1]]$forecasts), 
                                             matrix(g1_A2[[1]]$LM_MAE[[1]]$forecasts),
                                             matrix(g1_A2[[1]]$ELN_MSE[[1]]$forecasts),
                                             matrix(g1_A2[[1]]$ELN_MAE[[1]]$forecasts))),
                         q = 10, loss.type = "SE")
