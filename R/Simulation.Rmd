...
```{r tune_characteristics_rqsuared}
#Tuning time series R Squared
#Tuning v vector + error variance
#Want average time series for each characterisics r squared to be 50%, average annual volatility to be 30%

#Generate paramaters first

beta <- gen_Beta()

#variance of the errors is what we want to grid search
#Gu et al had 0.05 for their v variance, that does not work at all here
v <- gen_v(5)
...
for (i in 1:N){
  ...
  for (t in 1:(Time+1)){
    char_tune_df$beta_v[t] <- beta[[t]][i, ] %*% v[[t]]
    char_tune_df$error[t] <- error[[t]][i]
  }
  
  #Cbind returns, id, time and predictors
  panel <- cbind(char_tune_df$beta_v, char_tune_df$error, c(1:(Time+1)), c(rep(i, (Time+1))))
  char_tune_panel[((i-1)*(Time+1)+1):(i*(Time+1)), ] <- panel
  
}
  
#Sort by time
char_tune_panel <- char_tune_panel %>%
  arrange(time)
...
SSR <- t(char_tune_panel$error) %*% (char_tune_panel$error)
SST <- t(char_tune_panel$beta_v - mean(char_tune_panel$beta_v)) %*% (char_tune_panel$beta_v - mean(char_tune_panel$beta_v))
...
for (i in 1:200) {
  vol_tune_panel <- char_tune_panel %>%
    filter(stock == i)
  vol_tune_df$annual_vol[i] <- sd(vol_tune_panel$beta_v) * sqrt(12)
}
...
for (i in 1:N){
  ...
  rt_tune_df <- data.frame(
    return = rep(0, Time+1),
    resid = rep(0, Time+1)
  )
  
  for (t in 1:(Time+1)){
    rt_tune_df$return[t] <- rt[[t]][i]
    rt_tune_df$resid[t] <- resid[[t]][i]
  }
  
  #Cbind returns, id, time and predictors
  panel <- cbind(rt_tune_df$return, rt_tune_df$resid, c(1:(Time+1)))
  rt_tune_panel[((i-1)*(Time+1)+1):(i*(Time+1)), ] <- panel
  
}
  
#Sort by time
rt_tune_panel <- rt_tune_panel %>%
  arrange(time)
...
SSR <- t(rt_tune_panel$resid) %*% (rt_tune_panel$resid)
SST <- t(rt_tune_panel$return - mean(rt_tune_panel$return)) %*% (rt_tune_panel$return - mean(rt_tune_panel$return))
