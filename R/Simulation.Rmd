...
#Original univariate xt function, just for simplicity
...
gen_Beta <- function(){
  #Beta is the first 3 columns of the characteristics matrix
  Beta <- C[, 1:3, ]
  return(Beta)
}
...
gen_v <- function(v_sd){
  v <- matrix(data = rnorm(3*(Time+1),mean=0, sd = v_sd), 
              nrow = (Time+1), 
              ncol = 3)
  return(v)
}
...
gen_error <- function(omega, gamma, w){
  ...
  sigma2 <- array(data = 0, dim = Time+1)
  ...
  ##No Stochastic Volatility for now
}

#Shit is super complicated here, maybe nuke and restart?
