...
#C_hat are the characteristics that contain cross sectional correlation
...
g1 <- function(C, x, i, t, theta){
  ...
  matrix(c(
    C[[t]][i,1], C[[t]][i,2], C[[t]][i,3] * t(x[t])
    ), nrow = 1) %*% t(theta)
}

#test if working, working
g1(C, gu_xt, 1, 1, theta)
...
```{r gu_generate_return_equations}
##########################
##Functions for elements in return equation
##########################
...
#Errors under Gu specification are very different (much simpler)
...
gen_error <- function(ep_sd){
  ...
  for (t in 1:Time+1){
    for (i in 1:N) {
      #This is how you build in a specified variance
...
gu_gen_rt_ep_error <- function(ep_sd){
  elp <- matrix(0, nrow = N, ncol = 1)
  
  ep_error <- rep(list(elp), Time+1)
...
gu_gen_rt_betav <- function(v_sd, ep_error){
  elp <- matrix(0, nrow = N, ncol = 1)
  
  betav <- rep(list(elp), Time+1)
  ...
  for (t in 1:(Time)) {
    for (i in 1:N){
      betav[[t+1]][i] <- Beta[[t+1]][i, ] %*% v[[t+1]] + ep_error[[t+1]][i]
    }
  }
...
```{r tune_rsquared_skeleton, eval = FALSE}
