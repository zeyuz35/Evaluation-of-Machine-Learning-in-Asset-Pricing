...
gen_C_bar <- function () {
  ...
  for (t in 1:(Time+1)) {
    for (jj in 1:P_c) {
      ...
      C_bar[[t+1]][, jj] <- (C_bar[[t]][, jj]*rho[jj] + rnorm(N, 0, 1))
    }  
  }
...
gu_gen_error <- function(ep_sd){
  ...
  for (t in 1:Time+1){
    for (ii in 1:N) {
      ...
      error[[t]][ii] <- rt(1, df = 5)*sqrt(ep_sd^2 * (5-2)/5)
    }
  }
...
gu_gen_rt_resid <- function(v_sd, ep_sd){
  ...
  for (t in 1:(Time)) {
    for (ii in 1:N){
      resid[[t+1]][ii] <- Beta[[t+1]][ii, ] %*% v[[t+1]] + error[[t+1]][ii]
    }
  }
  return(resid)
}
...
gu_gen_rt <- function(g, resid){
  ...
  for (t in 1:(Time)) {
    for (ii in 1:N){
      rt[[t+1]][ii] <- g(C, gu_xt, ii, t, theta) + resid[[t+1]][ii]
    }
  }
...
for (ii in 1:N) {
  rt_id_tune_panel <- rt_cross_tune_panel |>
    filter(stock == ii)
  rt_id_tune_df$rsquare[ii] <- r_squared_proper(rt_id_tune_panel$g, rt_id_tune_panel$return)
}
...
for (ii in 1:N) {
  vol_tune_panel <- rt_cross_tune_panel |>
    filter(stock == ii)
  vol_tune_df$annual_vol[ii] <- sd(vol_tune_panel$return) * sqrt(12)
}
