...
gu_gen_xt <- function(){
  ...
  for (i in 1:(Time+1)) {
    ut <- rnorm(1, mean = 0, sd = sqrt((1-p^2)))
    xt[1+i] <- p*xt[i] + ut
  }
  #Remove Initial
  xt <- xt[-1]
...
#What the shit Gu, this isn't very persistent at all
...
#Initialize a data frame to store our ultimate panel data in
...
for (i in 1:N){
  #Store the residuals and g into it
  for (t in 1:(Time+1)){
    rt_cross_tune_df$resid[t] <- resid[[t]][i]
    rt_cross_tune_df$g[t] <- g[[t]][i]
    rt_cross_tune_df$time <- t
    rt_cross_tune_df$stock <- i
  }
  rt_cross_tune_panel <- rbind(rt_cross_tune_panel, rt_cross_tune_df)
}

for (t in (Time+1)) {
  rt_cross_tune_panel <- cbind(resid = resid[[t]], g = g[[t]], time = rep(t, N), stock = rep(1, N))
}
...
rt_cross_tune_panel <- rt_cross_tune_panel %>%
  ...
  mutate(return_sq = (return-mean(return)^2)) %>%
...
for (i in 1:N) {
  rt_id_tune_panel <- rt_cross_tune_panel %>%
    filter(stock == i)
  SSR <- sum(rt_id_tune_panel$resid_sq)
  SST <- sum(rt_id_tune_panel$return_sq)
...
SSR <- rt_cross_tune_panel_cross_section$resid_sq
SST <- rt_cross_tune_panel_cross_section$return_sq

1 - SSR/SST

#Predictive R Squared, across the entire cross section and all time

SSR <- sum(rt_cross_tune_panel$resid_sq)
SST <- sum(rt_cross_tune_panel$return_sq)
