...
macro_predictors_ff <- F_F_Research_Data_5_Factors_2x3_RAW %>%
  data.frame() %>%
  ## Filtering out the extra two factors for feasibility and singularity issues
  dplyr::select(Mkt.RF, SMB, HML, Time) %>%
  # Rename
  mutate(time = Time) %>%
  dplyr::select(-Time) %>%
  mutate(time = as.yearmon(time)) %>%
...
LM_fit <- function(pooled_panel, timeSlices, loss_function, f, 
  ...
  for (set in 1:3) {
    ...
    if (loss_function == "mse") {
      ...
      lm <- rq(f, data = train, tau = 0.5, method = "fn")
    }
...
```{r, eval = FALSE}
# test
## Some problems. With more factors (as is with fama french setup) some of the quantile models are singular and have no fit due to the extra dimensionality
## Solution: use jitter() to add a small amount of noise to the response variable
