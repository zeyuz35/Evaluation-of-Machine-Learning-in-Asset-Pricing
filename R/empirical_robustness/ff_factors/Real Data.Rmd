...
# Note that because the honours lab computers do not have their own hard drives (files are stored on some network location), this initial loading processis very slow (bottlenecked by network speed)
datashare_RAW <- read_csv("~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/data/datashare/datashare.csv",
                          # Always make sure to force col_types so that read_csv does not assume weird stuff
...
datashare <- datashare_RAW_join %>%
  ...
  # Do not have any idea why Gu et al thought this was originally sensible
  ...
  # Also do not make much consistent sense either
...
# Do not know how Gu et al did this
...
PredictorData2017 <- PredictorData2017_RAW %>%
  ...
  # Note that log(D12) - log(Index) is negative which does not make sense as a ratio
  mutate(dp = abs(log(D12) - log(Index))) %>%
  # earnings price ratio is the difference between the log of earnings and the log of prices
  # Note that log(E12) - log(Index) is negative which does not make sense as a ratio
...
# Also filter out all the entries that are not in the datashare dataset as they cannot be used (Note that this also takes care of all missing values in this dataset)
# Filter it so that we only have the years we are interested in (same filtering as datashare)
macro_predictors_wg <- PredictorData2017 %>%
...
F_F_Research_Data_5_Factors_2x3_RAW <- read_csv("~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/data/factors/F-F_Research_Data_5_Factors_2x3.csv",
                                                skip = 3, n_max = 671) %>%
  mutate(Time = as.yearmon(as.character(X1), "%Y%m")) %>%
  select(-X1)

macro_predictors_ff <- F_F_Research_Data_5_Factors_2x3_RAW %>%
  data.frame() %>%
  dplyr::select(Mkt.RF, SMB, HML, CMA, RMW, Time) %>%
  # Rename
  mutate(time = Time) %>%
  dplyr::select(-Time) %>%
  mutate(time = as.yearmon(time)) %>%
  # Convert time to quarterly format 
  filter((as.yearmon(time) %% 1) == 2/12 | (as.yearmon(time) %% 1) == 5/12 | (as.yearmon(time) %% 1) == 8/12 | (as.yearmon(time) %% 1) == 11/12) %>%
  # Lag Variables
  mutate(Mkt.RF = lag(Mkt.RF)) %>%
  mutate(SMB = lag(SMB)) %>%
  mutate(HML = lag(HML)) %>%
  mutate(RMW = lag(RMW)) %>%
  mutate(CMA = lag(CMA)) %>%
  # Rename
  rename_at(vars(-time), function(x) paste0("macro_ff_", x)) %>%
  # Create constant
  # Reorder
  dplyr::select(time, everything()) %>%
  filter(time >= 1993.5 & time < 2017) %>%
...
# Combine FF and WG factors together

macro_predictors <- macro_predictors_ff %>% 
  full_join(macro_predictors_wg, by = "time")

macro_predictors <- macro_predictors %>%
  dplyr::select(macro_constant, everything())
```
...
LM_variable_importance <- function(test, lm_model, 
  ...
  # Specify .packages = "quantreg" here as it seems it is not supported and therefore is missed by doFuture
...
## Change these to be dummy functions that just return 0 for computational feasibility
## This was originally just implemented so that the stupid diebold mariano tests could be done
## DM tests are either unreliable and have numerical issues, or just tell you what you can already decipher from loss stats anyway
...
LM_fit <- function(pooled_panel, timeSlices, loss_function, f, 
  ...
  for (set in 1:3) {
    ...
    # Set model = FALSE so that it will not save a copy of the training data in the model object
    ...
    if (loss_function == "mse") {
      ...
      lm <- rq(f, data = train, tau = 0.5, method = "pfn", eps = 1e-02)
    }
...
NNet_fit_stats <- function(pooled_panel, timeSlices, hidden_layers, loss_function, batch_size, patience,
  ...
  for (set in 1:3) {
    ...
    # Gu et al do not say what their early stopping parameter p is
    ...
    # Namely, batch size
    # In general, larger batch sizes result in faster progress in training, but do not always converge as fast. Smaller batch sizes train slower, but can converge faster.
...
LM_stats_mse <- LM_fit(final_dataset, realdata_timeSlices, "mse", f,
                       macro_factor_names, individual_factor_names)
saveRDS(LM_stats_mse, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/LM_stats_mse.rds")

LM_stats_mae <- LM_fit(final_dataset, realdata_timeSlices, "mae", f,
                       macro_factor_names, individual_factor_names)
saveRDS(LM_stats_mae, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/LM_stats_mae.rds")
...
LM_stats_mae[[3]]$variable_importance %>%
  arrange(desc(importance))
######################################################################################################################
ELN_stats_mse <- ELN_fit_stats(alpha_grid = seq(0.01, 1, 0.01), nlamb = 100, 
                               realdata_timeSlices, final_dataset, "mse",
                               macro_factor_names, individual_factor_names)
saveRDS(ELN_stats_mse, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/ELN_stats_mse.rds")
ELN_stats_mae <- ELN_fit_stats(alpha_grid = seq(0.01, 1, 0.01), nlamb = 100, 
                               realdata_timeSlices, final_dataset, "mae",
                               macro_factor_names, individual_factor_names)
saveRDS(ELN_stats_mae, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/ELN_stats_mae.rds")
...
# Random Forests
RF_grid <- expand.grid(
  #ntree usually is not tuned. Just set to max of computationally feasible
  ntree = 50,
...
RF_mse_stats <- RF_fit_stats(final_dataset, RF_grid, realdata_timeSlices, "mse", f,
                             macro_factor_names, individual_factor_names)
saveRDS(RF_mse_stats, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/RF_mse_stats_1.5_train_valid.rds")
RF_mae_stats <- RF_fit_stats(final_dataset, RF_grid, realdata_timeSlices, "mae", f,
                             macro_factor_names, individual_factor_names)
saveRDS(RF_mae_stats, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/RF_mae_stats_1.5_train_valid.rds")
...
NNet_1_mse_stats <- NNet_fit_stats(final_dataset, realdata_timeSlices, 1, "mse", batch_size = 256, 20,
                                   macro_factor_names, individual_factor_names)
NNet_1_mae_stats <- NNet_fit_stats(final_dataset, realdata_timeSlices, 1, "mae", batch_size = 256, 20,
                                   macro_factor_names, individual_factor_names)
saveRDS(NNet_1_mse_stats, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/NNet_1_mse_stats_1.5.rds")
saveRDS(NNet_1_mae_stats, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/NNet_1_mae_stats_1.5.rds")
...
NNet_2_mae_stats <- NNet_fit_stats(final_dataset, realdata_timeSlices, 2, "mae", batch_size = 256, 20,
                                   macro_factor_names, individual_factor_names)
saveRDS(NNet_2_mse_stats, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/NNet_2_mse_stats_1.5.rds")
saveRDS(NNet_2_mae_stats, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/NNet_2_mae_stats_1.5.rds")
...
NNet_3_mae_stats <- NNet_fit_stats(final_dataset, realdata_timeSlices, 3, "mae", batch_size = 256, 20,
                                   macro_factor_names, individual_factor_names)
saveRDS(NNet_3_mse_stats, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/NNet_3_mse_stats_1.5.rds")
saveRDS(NNet_3_mae_stats, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/NNet_3_mae_stats_1.5.rds")
...
NNet_4_mae_stats <- NNet_fit_stats(final_dataset, realdata_timeSlices, 4, "mae", batch_size = 256, 20,
                                   macro_factor_names, individual_factor_names)
saveRDS(NNet_4_mse_stats, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/NNet_4_mse_stats_1.5.rds")
saveRDS(NNet_4_mae_stats, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/NNet_4_mae_stats_1.5.rds")
...
NNet_5_mae_stats <- NNet_fit_stats(final_dataset, realdata_timeSlices, 5, "mae", batch_size = 256, 20,
                                   macro_factor_names, individual_factor_names)
saveRDS(NNet_5_mse_stats, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/NNet_5_mse_stats_1.5.rds")
saveRDS(NNet_5_mae_stats, file = "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/R/Model_results/empirical_ff/NNet_5_mae_stats_1.5.rds")
