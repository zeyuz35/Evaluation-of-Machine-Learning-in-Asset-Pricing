...
macro_predictors_ff <- F_F_Research_Data_5_Factors_2x3_RAW %>%
  data.frame() %>%
  dplyr::select(Mkt.RF, SMB, HML, RMW, CMA, Time) %>%
  # Rename
  mutate(time = Time) %>%
  dplyr::select(-Time) %>%
  mutate(time = as.yearmon(time)) %>%
...
```{r}
## This section builds a version of the final dataset which does NOT have any interaction terms
## This was mainly done to reduce dimensionality
...
saveRDS(datashare_combined, file = "final_dataset_no_interaction.rds")
```
