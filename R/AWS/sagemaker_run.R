...
## Therefore, provide it with the training + validation set data (assuming this is not too computationally intensive)
...
##
## Get the prediction from S3 and turn them into a data frame which we can use

##############################################################
## Giant wrapper for all of the above
##############################################################
...
deepar_fit_stats <- function(pooled_panel, timeSlices) {
  ...
  for (set in 1:3) {
    ...
    LSTM_stats[[set]] <- list(loss_stats = data.frame(train_MAE = 0, train_MSE = 0, train_RMSE = 0, train_RSquare = 0, 
                              ...
                              #Other useful things
                              # Keep forecasts here in for nnet objects to make sure they are not doing something stupid
                              forecasts = 0,
                              forecast_resids = 0,
                              model = 0,
    ...
    ## Create a sagemaker estimator (ie fit the model)
