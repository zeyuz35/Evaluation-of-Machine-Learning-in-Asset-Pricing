...
deepar_fit_stats <- function(pooled_panel, timeSlices) {
  ...
  for (set in 1:3) {
    ## Important Things
    DEEPAR_stats[[set]] <- list(
      loss_stats = data.frame(
        train_MAE = 0, train_MSE = 0, train_RMSE = 0, train_Rsquare = 0,
    ...
    transformer <- estimator$transformer(
      instance_count = 1L, instance_type = "ml.c4.xlarge", output_path = batch_output,
...
#' Fit DeepAR Models
#'
#' Fits deep autoregressive models using Amazon SageMaker across a range of datasets.
#'
#' @param dataset_list A \code{list} containing datasets with pooled panel structures.
#' @param batch_process_range A \code{numeric} vector indicating the batches to process.
#' @return A \code{list} of simulation results containing model statistics and forecasts.
