...
treeInteractions <- function(input_tree, input_max_depth){
  ...
  # Attach parent nodes
  for (max_depth_ii in 2:input_max_depth){
    if (max_depth_ii == 2) trees[, ID_merge:=ID] else trees[, ID_merge:=get(paste0('parent_',max_depth_ii-2))]
    ...
    trees[!is.na(i.id), c(paste0('parent_', max_depth_ii-1), paste0('parent_feat_', max_depth_ii-1)):=list(i.id, i.feature)]
