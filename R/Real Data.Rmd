...
## Function to generate log quarterly returns
## This does not account for stocks entering and then leaving for whatever reason
## Therefore not working properly yet
...
datashare <- foreach(i = (1:length(unique(datashare$permno))), .combine = rbind) %dopar% {
  datashare_stock <- datashare %>%
    ...
    arrange(Time)
  # This if statement filters out stocks which do not appear continuously throughout the dataset
  if (mean((datashare_stock$Time - lag(datashare_stock$Time))[-1] == 1/4) == 1) {
    datashare_stock %>%
      mutate(RET_M = (logPRC - lag(logPRC)))
  }
}

datashare_test <- datashare %>%
  filter(permno == 11587) %>%
  arrange(Time)

(datashare_test$Time - lag(datashare_test$Time))[-1] == 1/4)

datashare %>%
  drop_na(RET_M) %>%
  select(Time, permno, PRC, logPRC, RET_M, everything()) %>%
...
# This smaller than our dataset, so make sure to set it to something large
...
test <- cross_sectional_values(datashare, impute_type = "mean")
