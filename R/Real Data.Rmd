...
combined <- full_join(macro_predictors, datashare_normalized, by = "time") %>%
...
final_dataset <- data.frame(combined$stock, combined$time, combined$rt, final_dataset_x) %>%
  # Rename some columns
  mutate(stock = combined.stock) %>%
  mutate(time = combined.time) %>%
  mutate(rt = combined.rt) %>%
  dplyr::select(-combined.stock, -combined.time, -combined.rt) %>%
  dplyr::select(time, stock, rt, everything())
...
# General plan:
# Supply a combined_dataset TEST set with no interaction terms, a formula from care tto be able to generate those interaction terms, and a previously estimated model object
# For each column, excluding macro_constant, set it to zero
# Put it through predict(f) to get the interaction terms
# Calculate out of sample r squared and hence variable importance
...
RF_variable_importance <- function(test, rf_model) {
  ...
  variable_importance_df <- foreach(i = (1:ncol(test_x)), .combine = "rbind", .packages = "randomForestSRC") %dopar% {
    test_x_zero <- test_x
    test_x_zero[, i] <- 0
...
fit_all_models_real_data <- function(pooled_panel,
  ...
  # Load pooled_panel
  real_data_results_list$returns <- pooled_panel$rt
