  ...
  if (g_name == "g1") {
    ...
    for (t in 1:Time) {
      ...
      if (nrow(x) == 1) {
        ...
        g_factor_panel[, 3, t] <- C[, 3, t] * x[1, t]
      }
  ...
  #G2 Case
    ...
    for (t in 1:Time) {
      ...
      if (nrow(x) == 1) {
        ...
        g_factor_panel[, 3, t] <- sign(C[, 3, t] * x[1, t])
      }
      #Multivariate xt Case
      else {
        g_factor_panel[, 1, t] <- C[, 1, t]^2
...
gen_predictor_z <- function(C, x){
  ...
  for (t in 1:Time) {
    z_panel[, , t] <- kronecker(t(xt_set[, t]), C[, , t], make.dimnames = FALSE)
  }
  return(z_panel)
}
...
bind_rt_predictor <- function(rt_panel, z_panel) {
  ...
  df_list <- vector("list", Time)
  stock_labels <- paste0("stock_", c(1:N))
