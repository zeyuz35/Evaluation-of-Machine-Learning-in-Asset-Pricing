...
gen_g_factor_panel <- function(g_function, C, x) {
  ...
  if (g_name == "g1") {
    ...
    for (t in 1:Time) {
      #Univariate xt Case
      if (nrow(x) == 1) {
        g_factor_panel[, 1, t] <- C[, 1, t]
        g_factor_panel[, 2, t] <- C[, 2, t]
        g_factor_panel[, 3, t] <- C[, 3, t] * x[, t]
      } else {
      #Multivariate xt Case
        g_factor_panel[, 1, t] <- C[, 1, t]
        g_factor_panel[, 2, t] <- C[, 2, t]
        g_factor_panel[, 3, t] <- C[, 3, t] * x[3, t]
      }
    }
  } else if (g_name == "g2") { #G2 Case
    ...
    for (t in 1:Time) {
      #Univariate xt Case
      if (nrow(x) == 1) {
        g_factor_panel[, 1, t] <- C[, 1, t]^2
        g_factor_panel[, 2, t] <- C[, 1, t] * C[, 2, t]
        g_factor_panel[, 3, t] <- sign(C[, 3, t] * x[, t])
      } else {
      #Multivariate xt Case
        g_factor_panel[, 1, t] <- C[, 1, t]^2
        g_factor_panel[, 2, t] <- C[, 1, t] * C[, 2, t]
        g_factor_panel[, 3, t] <- sign(C[, 3, t] * x[3, t])
      }
    }
  } else if (g_name == "g3") { #G3 Case
    ...
    for (t in 1:Time) {
      g_factor_panel[, 1, t] <- (C[, 1, t] > 0)
      g_factor_panel[, 2, t] <- C[, 2, t]^3
      g_factor_panel[, 3, t] <- C[, 1, t] * C[, 2, t] * (C[, 3, t] > 0)
      g_factor_panel[, 4, t] <- logit(C[, 3, t])
    }
  } else if (g_name == "g4") { #G4 Case
...
gen_g_panel <- function(g_factor, theta) {
  ...
  for (t in 1:Time) {
    g_panel[, 1, t] <- g_factor[, , t] %*% t(theta)
  }
  return(g_panel)
}
...
sim_panel_data <- function(sim_N, 
  ...
  sim_list <- foreach(i = (1:sim_N)) %dopar% {
    ...
    } else { # Original Kronecker specification (set up in else so that it\'s the default)
