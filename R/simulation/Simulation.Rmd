...
gen_W <- function(lambda_degree){
  ...
  # Optimized computation: tcrossprod(Lambda) is equivalent to Lambda %*% t(Lambda) but faster
  B <- tcrossprod(Lambda)
...
gen_g_panel <- function(g_factor, theta) {
  ...
  for (ii in 1:N) {
    for (tt in 1:Time) {
      # Optimized computation: tcrossprod is equivalent to x %*% t(y)
      g_panel[ii, 1, tt] <- tcrossprod(g_factor[ii, , tt], theta)
    }
  }
  return(g_panel)
}
