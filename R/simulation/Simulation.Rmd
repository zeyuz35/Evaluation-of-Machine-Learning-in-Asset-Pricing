...
gen_C_bar <- function(rho_a, rho_b){
  ...
  #provideDimnames does not do exactly what I want it to
...
gen_error <- function(sv, 
  ...
  for (tt in 1:(Time)) {
    v <- matrix(data = rnorm(3, mean = 0, sd = v_sd), 
                nrow = 3, 
                ncol = 1)
    # Vectorized matrix multiplication: equivalent to Beta_v[ii, 1, tt] <- Beta[ii, , tt] %*% v
    Beta_v[, 1, tt] <- Beta[, , tt] %*% v
  }
  ###########
  #SV errors
  ###########
  if (sv == 1) {
    ...
    #Initial sigma2
    logsigma2[1] <- omega/(1-gamma)
    
    for (tt in 2:(Time)) {
      logsigma2[tt] <- omega + gamma*logsigma2[tt-1] + rnorm(1, 0, w)
    }
    
    for (tt in 1:(Time)) {
      # Vectorized normal error generation: equivalent to iterating over N for ii
      error[, 1, tt] <- sqrt(exp(logsigma2[tt])) * rnorm(N, 0, ep_sd)
    }
    return(Beta_v + error)
  }
  ##################
  ...
  ##################
    ...
    for (tt in 1:(Time)) {
      error[, , tt] <- matrix(data = rt(N, df = 5) * sqrt(ep_sd^2 * (5-2)/5), nrow = N)
    }
    return(error + Beta_v)
  }
}
...
#You will still need to specify values for omega, gamma and w, they just will not be used
...
# This is highly performant, finishing 100 simulations in less than 5 minutes on 12 logical cores

sim_panel_data <- function(sim_N, 
                           char_rho_a, char_rho_b,
                           cross_corr, cross_corr_degree, 
                           A_matrix, xt_multi, g_function, theta,
                           #Error Parameters
...
# Gu et al's design does not seem to be replicable
...
sim_panel_data <- function(sim_N, cross_corr, cross_corr_dergee, A_matrix, xt_multi, g_function, theta,
                           #Error Parameters
                           error_sv, error_ep_sd, error_omega, error_gamma, error_w, error_v_sd) {
  #Initialize
...
# A matrices
# These are just kept here for now, not actually used because they do not seem to have much difference
...
gu_et_al_g1 <- sim_panel_data(simN, 
                              ...
                              cross_corr = 0, cross_corr_degree = 0, 
                              A1, xt_multi = 0, g_function = "g1", theta = matrix(c(0.02, 0.02, 0.02), nrow = 1),
...
gu_et_al_g2 <- sim_panel_data(simN, 
                              char_rho_a = 0.9, char_rho_b = 1,
                              cross_corr = 0, cross_corr_degree = 0, 
                              A1, xt_multi = 0, g_function = "g2", theta = matrix(c(0.04, 0.035, 0.01), nrow = 1),
                              error_sv = 0, error_ep_sd = 0.05, 
                              error_omega = -0.736, error_gamma = 0.9, error_w = 0.363, error_v_sd = 0.05,
                              predictor_format = "kronecker")
...
# For some reason the SV parameters suggested by JPR 1994 (and in the literature) does not give ~20% annualized volatility with this returns simulation design, even when re-specifying everything to be weekly
...
## No cross sectional correlation or stochastic errors

a4_aspect_ratio <- 1.5

#G1
g1_A1_nosv_0 <- sim_panel_data(simN, 
                              char_rho_a = 0.5, char_rho_b = 1,
                              cross_corr = 0, cross_corr_degree = 0.01, 
                              A1, xt_multi = 1, g_function = "g1", theta = matrix(c(0.02, 0.02, 0.02), nrow = 1), 
                              error_sv = 0, error_ep_sd = 0.05, 
                              error_omega = -0.736, error_gamma = 0.9, error_w = 0.363, error_v_sd = 0.05,
                              predictor_format = "kronecker")
summary(sim_tune_statistics(g1_A1_nosv_0))
saveRDS(g1_A1_nosv_0, file = "g1_A1_nosv_0.rds")
rm(g1_A1_sv_0.01)

## Plot this to see how unreasonable this is

g1_A1_nosv_0[[1]]$panel %>%
  ggplot() +
  geom_line(aes(x = time, y = rt, colour = stock)) +
  theme_bw() +
  theme(legend.position = "none")
ggsave("~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/Results/g1_A1_nosv_panel.pdf",
       width = 15 * a4_aspect_ratio, height = 15, units = "cm")
  

#G2

g2_A1_nosv_0 <- sim_panel_data(simN, 
                              char_rho_a = 0.5, char_rho_b = 1,
                              cross_corr = 0, cross_corr_degree = 0.01, 
                              A1, xt_multi = 1, g_function = "g2", theta = matrix(c(0.015, 0.015, 0.015), nrow = 1),
                              error_sv = 0, error_ep_sd = 0.05, 
                              error_omega = -0.736, error_gamma = 0.9, error_w = 0.363, error_v_sd = 0.05,
                              predictor_format = "kronecker")
summary(sim_tune_statistics(g2_A1_nosv_0))
saveRDS(g2_A1_nosv_0, file = "g2_A1_nosv_0.rds")
rm(g2_A1_sv_0.01)

#G3

g3_A1_nosv_0 <- sim_panel_data(simN, 
                              char_rho_a = 0.5, char_rho_b = 1,
                              cross_corr = 0, cross_corr_degree = 0.01,
                              A1, xt_multi = 1, g_function = "g3", theta = matrix(c(0.025, 0.025, 0.025, 0.025), nrow = 1), 
                              error_sv = 0, error_ep_sd = 0.05, 
                              error_omega = -0.736, error_gamma = 0.9, error_w = 0.363, error_v_sd = 0.05,
                              predictor_format = "kronecker")
summary(sim_tune_statistics(g3_A1_nosv_0))
saveRDS(g3_A1_nosv_0, file = "g3_A1_nosv_0.rds")
rm(g3_A1_sv_0.01)

####################################################
## 0.01 Cross Sectional Correlation

#G1
g1_A1_sv_0.01 <- sim_panel_data(simN, 
                              char_rho_a = 0.5, char_rho_b = 1,
                              cross_corr = 1, cross_corr_degree = 0.01, 
                              ...
                              predictor_format = "kronecker")
summary(sim_tune_statistics(g1_A1_sv_0.01))
saveRDS(g1_A1_sv_0.01, file = "g1_A1_sv_0.01.rds")
rm(g1_A1_sv_0.01)

g1_A1_sv_0.01[[2]]$panel %>%
  ggplot() +
  geom_line(aes(x = time, y = rt, colour = stock)) +
  theme(legend.position = "none")
...
g2_A1_sv_0.01 <- sim_panel_data(simN, 
                              char_rho_a = 0.5, char_rho_b = 1,
                              cross_corr = 1, cross_corr_degree = 0.01, 
                              ...
                              error_omega = -0.275, error_gamma = 0.95, error_w = 0.430, error_v_sd = 0,
                              predictor_format = "kronecker")
summary(sim_tune_statistics(g2_A1_sv_0.01))
saveRDS(g2_A1_sv_0.01, file = "g2_A1_sv_0.01.rds")
rm(g2_A1_sv_0.01)

g2_A1_sv_0.01[[2]]$panel %>%
  ggplot() +
  geom_line(aes(x = time, y = rt, colour = stock)) +
  theme(legend.position = "none")
...
g3_A1_sv_0.01 <- sim_panel_data(simN, 
                              char_rho_a = 0.5, char_rho_b = 1,
                              cross_corr = 1, cross_corr_degree = 0.01,
                              ...
                              error_omega = -0.275, error_gamma = 0.95, error_w = 0.430, error_v_sd = 0,
                              predictor_format = "kronecker")
summary(sim_tune_statistics(g3_A1_sv_0.01))
saveRDS(g3_A1_sv_0.01, file = "g3_A1_sv_0.01.rds")
rm(g3_A1_sv_0.01)

g3_A1_sv_0.01[[2]]$panel %>%
  ggplot() +
  geom_line(aes(x = time, y = rt, colour = stock)) +
  theme(legend.position = "none")

#############################
## 0.1 Cross Sectional Correlation

#G1
g1_A1_sv_0.1 <- sim_panel_data(simN, 
                              char_rho_a = 0.5, char_rho_b = 1,
                              cross_corr = 1, cross_corr_degree = 0.1, 
                              A1, xt_multi = 1, g_function = "g1", theta = matrix(c(0.015, 0.015, 0.015), nrow = 1), 
                              error_sv = 1, error_ep_sd = 1, 
                              error_omega = -0.275, error_gamma = 0.95, error_w = 0.430, error_v_sd = 0,
                              predictor_format = "kronecker")
summary(sim_tune_statistics(g1_A1_sv_0.1))
saveRDS(g1_A1_sv_0.1, file = "g1_A1_sv_0.1.rds")
rm(g1_A1_sv_0.1)

g1_A1_sv_0.1[[2]]$panel %>%
  ggplot() +
  geom_line(aes(x = time, y = rt, colour = stock)) +
  theme(legend.position = "none")
...
g2_A1_sv_0.1 <- sim_panel_data(simN, 
                              char_rho_a = 0.5, char_rho_b = 1,
                              cross_corr = 1, cross_corr_degree = 0.1, 
                              A1, xt_multi = 1, g_function = "g2", theta = matrix(c(0.012, 0.012, 0.012), nrow = 1),
                              error_sv = 1, error_ep_sd = 1, 
                              error_omega = -0.275, error_gamma = 0.95, error_w = 0.430, error_v_sd = 0,
                              predictor_format = "kronecker")
summary(sim_tune_statistics(g2_A1_sv_0.1))
saveRDS(g2_A1_sv_0.1, file = "g2_A1_sv_0.1.rds")
rm(g2_A1_sv_0.1)

g2_A1_sv_0.1[[2]]$panel %>%
  ggplot() +
  geom_line(aes(x = time, y = rt, colour = stock)) +
  theme(legend.position = "none")
...
g3_A1_sv_0.1 <- sim_panel_data(simN, 
                              char_rho_a = 0.5, char_rho_b = 1,
                              cross_corr = 1, cross_corr_degree = 0.1,
                              A1, xt_multi = 1, g_function = "g3", theta = matrix(c(0.02, 0.02, 0.02, 0.02), nrow = 1), 
                              error_sv = 1, error_ep_sd = 1, 
                              error_omega = -0.275, error_gamma = 0.95, error_w = 0.430, error_v_sd = 0,
                              predictor_format = "kronecker")
summary(sim_tune_statistics(g3_A1_sv_0.1))
saveRDS(g3_A1_sv_0.1, file = "g3_A1_sv_0.1.rds")
rm(g3_A1_sv_0.1)

g3_A1_sv_0.1[[3]]$panel %>%
  ggplot() +
  geom_line(aes(x = time, y = rt, colour = stock)) +
  theme(legend.position = "none")
...
g1_A1_sv_1[[1]]$panel %>%
  ...
  theme_bw() +
  theme(legend.position = "none")
ggsave("~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/Results/g1_A1_sv_panel.pdf",
       width = 15 * a4_aspect_ratio, height = 15, units = "cm")
