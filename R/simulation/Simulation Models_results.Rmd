...
model_sam <- model_sam %>%
  ...
  mutate(model = cell_spec(model, angle = 90, format = "latex"))
...
test_loss_latex <- cbind(model_sam, cross_corr_sam, 
  ...
  kable(align = "c", format = "latex", digits = 6, escape = F) %>%
  kable_styling(full_width = F) %>%
  add_header_above(c(" " = 1, " " = 1, "g1" = 3, "g2" = 3, "g3" = 3)) %>%
  collapse_rows(columns = c(1:2), valign = "top")

test_loss_latex %>%
  cat("~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/Results/simulation/test_loss_latex.tex")
