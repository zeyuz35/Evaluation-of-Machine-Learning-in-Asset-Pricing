...
## Major bug with randomforestSRC - for some reason you need to epxlicitly pass forest = TRUE when calling rfsrc in order for vimp not to crash R, even though forest = TRUE should be the defualt
## Notes on computational problems: Breiman Cutler (permutation) is NOT feasible for vimp (took >3 hours and did not finish for a single rfsrc model)
## Increasing block size decreases computational burden - have to use something is Ishwaran-Kogalur
## Again, not actually too fussed with the specifics - just want to make sure that the results from a "usual" way of doing vimp is consistent
...
RF_fit_stats <- function(pooled_panel, RF_grid, timeSlices, loss_function, f) {
  ...
  for (set in 1:3) {
    ...
    test_vimp <- vimp(model, importance = "permute", block.size = 50)$importance
    ...
    #RF Built in VIMPS
    RF_stats[[set]]$vimp <- list(breiman_cutler = vimp(model, block.size = 1),
                                 ishwaran_kogalur = vimp(model, block.size = best_model_params$ntree))
  }
  return(RF_stats)
}
```
