...
# Given a single model and test set, return a variable importance dataframe (do not bother sorting them now)
...
## do not really believe in this, mostly done because its popular in literature
...
# cannot seem to find anything online that tunes ELN for you with specified validation sets, instead of straight-forward cross validation
...
# ELN is rather finicky and likes to generate its own sequence of lambda values during the actual fitting process. Really does not like specifying a singular lambda value
...
#forestr package does not seem to want to function on honours lab computers
...
## Just do them both, as they do not seem to be too intensive
...
## If you do not have enough memory, R will just crash and this is the major bug
...
# Stacking LSTM units does not seem to help too much
# Adding more vanilla layers also does not seem to help much
...
# Remeber that this function does not do train/validation/split, so you'll have to do these outside of it
...
## so that a constant model is not estimated
...
fforma_fit_stats <- function(pooled_panel, timeSlices, loss_function) {
  ...
  for (set in 1:length(fforma_fit_stats_list)) {
    ...
    ## Note that these forecasting methods do not really have a "training" error,
    ...
    ## and more importantly FFORMA does not care too much about external regressors anyway
