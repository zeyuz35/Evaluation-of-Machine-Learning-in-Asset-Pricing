...
empirical_loss_latex <- cbind(empirical_results_model, 
  ...
  add_header_above(c(" " = 1, "Sample 1" = 3, "Sample 2" = 3, "Sample 2" = 3)) %>%
...
ggsave(filename= "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/Results/empirical/empirical_all_sample_vi_ind.pdf", 
       width = 8, height = 8 * a4_aspect_ratio, units = "cm")
...
empirical_tidy_vi_macro %>%
  ...
  theme(legend.position = "right") +
  scale_fill_distiller(palette = "Spectral")
ggsave(filename= "~/GitHub/Evaluation-of-Machine-Learning-in-Asset-Pricing/Results/empirical/empirical_sample_all_vi_macro.pdf", 
...
vimp_iterate <- function(rf_results_list) {
  vimp_list <- foreach(set = 1:3, .combine = "rbind") %do% {
    bc <- rbind(cbind(sample = set, model = "RF_MSE", type = "bc",
                      data.frame(variable = names(rf_results_list[[set]]$vimp$breiman_cutler),
                                 importance = rf_results_list$RF_MSE[[set]]$vimp$breiman_cutler)), 
                cbind(sample = set, model = "RF_MAE", type = "bc",
                      data.frame(variable = names(rf_results_list$RF_MAE[[set]]$vimp$breiman_cutler),
                                 importance = rf_results_list$RF_MAE[[set]]$vimp$breiman_cutler))
                )
    
    ik <- rbind(cbind(sample = set, model = "RF_MSE", type = "ik", 
                      data.frame(variable = names(rf_results_list$RF_MSE[[set]]$vimp$ishwaran_kogalur),
                                 importance = rf_results_list$RF_MSE[[set]]$vimp$ishwaran_kogalur)),
                cbind(sample = set, model = "RF_MAE", type = "ik", 
                      data.frame(variable = names(rf_results_list$RF_MAE[[set]]$vimp$ishwaran_kogalur),
                                 importance = rf_results_list$RF_MAE[[set]]$vimp$ishwaran_kogalur))
    )
    rbind(bc, ik)
  }
