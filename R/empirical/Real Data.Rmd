...
for (ii in 1:length(macro_factor_names)) {
  interaction_terms[[ii]] <- paste(macro_factor_names[ii], individual_factor_names, sep = ":", collapse = " + ")
}
...
get_ELN_best_tune <- function(model_grid) {
  ...
  for (ii in 2:length(model_grid)) {
    ELN_tune_dataframe <- rbind(ELN_tune_dataframe, cbind(model_grid[[ii]]$ELN_grid, list_index = ii))
  }
...
RF_fit_model_grid <- function(f, train, validation, RF_grid, loss_function) {
  ...
  for (ii in 1:nrow(RF_grid)) {
    
    RF_model_grid[[ii]] <- list(RF_grid = 0, model = 0)
    
    #MSE Case
    if (loss_function == "mse") {
      RF <- rfsrc(f, train, 
                  #Hyperparameters
                  ntree = RF_grid$ntree[ii],
                  mtry = RF_grid$mtry[ii],
                  nodesize = RF_grid$nodesize[ii],
                  splitrule = "mse"
                  )
      
      #RF_model_grid[[ii]]$model <- RF
      RF_model_grid[[ii]]$RF_grid <- cbind(RF_grid[ii, ],
                                          #Train Loss
...
get_RF_best_tune <- function(RF_model_grid) {
  ...
  for (ii in 2:length(RF_model_grid)) {
    RF_tune_grid <- rbind(RF_tune_grid, RF_model_grid[[ii]]$RF_grid)
  }
