...
ELN_stats[[1]] <- list(loss_stats = c(train_MAE = 0, train_MSE = 0, train_RMSE = 0, train_RSquare = 0, 
...
ELN_model_grid <- function(ELN_grid) {
  #Initialize List
  ELN_model_grid <- rep(list(0), nrow(ELN_grid))
  
  for (i in 1:nrow(ELN_grid)) {
    ELN_tune_list[[i]] <- list(ELN_grid = ELN_grid[i,], model = 0, train_loss = 0, validation_loss = 0)
    #Remember to load glmnetUtils package for a formula interface to glmnet
    #Fit the model using grid value
    ELN <- glmnet(formula = f, data = train, alpha = ELN_tune_dataframe$alpha[i], lambda = ELN_tune_dataframe$lambda[i])
    ELN_tune_list[[i]]$model <- ELN
    #Training Error
    ELN_tune_list[[i]]$train_loss <- mse(train$rt, predict(ELN, newdata = train))
    #Validation Error
    ELN_tune_list[[i]]$validation_loss <- mse(validation$rt, predict(ELN, newdata = validation))
  }
...
get_ELN_best_tune <- function(tune_list) {
  ELN_tune_dataframe <- cbind(ELN_grid, train_loss = rep(0, nrow(ELN_grid)), validation_loss = rep(0, nrow(ELN_grid)))
  for (i in 1:nrow(ELN_grid)) {
    #Training Error
    ELN_tune_dataframe$train_loss[i] <- tune_list[[i]]$train_loss
    #Validation Error
    ELN_tune_dataframe$validation_loss[i] <- tune_list[[i]]$validation_loss
  }
  #Return the row index with the lowest validation loss
  return(which.min(ELN_tune_dataframe$validation_loss))
}

#Given the index of the grid which produces the best model, return an ELN_stats list with everything desired

gen_ELN_stats <- function(best_model_index, ELN_model_grid) {
  ELN_stats <- rep(list(0), 3)
  ELN_stats[[1]] <- list(loss_stats = data.frame(train_MAE = 0, train_MSE = 0, train_RMSE = 0, train_RSquare = 0, 
                         validation_MAE = 0, validation_MSE = 0, validation_RMSE = 0, validation_RSquare = 0, 
                         test_MAE = 0, test_MSE = 0, test_RMSE = 0, test_RSquare = 0),
                         #Other useful things
                         forecasts = 0,
                         ...
                         model = 0)
  #Model
  model <- ELN_model_grid$model
  ELN_stats[[1]]$model <- ELN_model_grid$model
  
  #Loss Stats Dataframe
  ELN_stats[[1]]$loss_stats$train_MAE <- mae(train$rt, predict(model, newdata = train))
  ELN_stats[[1]]$loss_stats$train_MSE <- mse(train$rt, predict(model, newdata = train))
  ELN_stats[[1]]$loss_stats$train_RMSE <- rmse(train$rt, predict(model, newdata = train))
  ELN_stats[[1]]$loss_stats$train_RSquare <- (1 - sse(train$rt, predict(model, newdata = train)) / sum((train$rt - mean(train$rt))^2))
    
  ELN_stats[[1]]$loss_stats$validation_MAE <- mae(validation$rt, predict(model, newdata = validation))
  ELN_stats[[1]]$loss_stats$validation_MSE <- mse(validation$rt, predict(model, newdata = validation))
  ELN_stats[[1]]$loss_stats$validation_RMSE <- rmse(validation$rt, predict(model, newdata = validation))
  ELN_stats[[1]]$loss_stats$validation_RSquare <- (1 - sse(validation$rt, predict(model, newdata = validation)) / sum((validation$rt - mean(validation$rt))^2))
  
  ELN_stats[[1]]$loss_stats$test_MAE <- mae(test$rt, predict(model, newdata = test))
  ELN_stats[[1]]$loss_stats$test_MSE <- mse(test$rt, predict(model, newdata = test))
  ELN_stats[[1]]$loss_stats$test_RMSE <- rmse(test$rt, predict(model, newdata = test))
  ELN_stats[[1]]$loss_stats$test_RSquare <- (1 - sse(test$rt, predict(model, newdata = test)) / sum((test$rt - mean(test$rt))^2))
  
  #Forecasts
  ELN_stats[[1]]$forecasts <- predict(model, newdata = test)
  
  #Forecast residuals
  ELN_stats[[1]]$forecasts <- test$rt - predict(model, newdata = test)
}
