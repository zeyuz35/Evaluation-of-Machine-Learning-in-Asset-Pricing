...
variable_importance <- data.frame(variable = colnames(test_x)[1], importance = (
  R2(predict(LM_stats_mae[[3]]$model, newdata = test_x), test$rt, form = "traditional") -
    R2(predict(LM_stats_mae[[3]]$model, newdata = test_x_zero), test$rt, form = "traditional")
  )
)
...
for (i in 2:ncol(test_x)) {
  ...
  variable_importance <- rbind(variable_importance, 
                               data.frame(variable = colnames(test_x)[i], 
                                          importance = (R2(predict(LM_stats_mae[[3]]$model, newdata = test_x), test$rt, form = "traditional") -
                                                                   R2(predict(LM_stats_mae[[3]]$model, newdata = test_x_zero), test$rt, form = "traditional")
                                                        )
                                          )
                               )
