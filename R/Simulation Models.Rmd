...
# Variable importance methodology doesn't seem to work
```
...
NNet_1_mse_stats <- NNet_fit_stats(pooled_panel, timeSlices, 1, "mse", batch_size = 512, 20)
...
```{r, eval = FALSE}
# Deprecated but still works, kept for legacy purposes
# New approach which makes everything neater: calculate variable importance metrics inside the model_fit_stats functions
# Variable Importance
# Variable Important is defined as the reduction in out of sample R squared from setting a factor's signal to 0

# Unfortunately all models have slightly different predict methods for their respective model objects, so unless we want to mess with that (no), we'll have to do slightly different VI functions for all different models
...
lm_variable_importance <- function(pooled_panel, timeSlices, model_fit_stats) {
  ...
  for (set in 1:3) {
    ...
    original_R2 <- R2(predict(model_fit_stats[[set]]$model, newdata = test_x), test$rt, form = "traditional")
    
    new_R2 <- R2(predict(model_fit_stats[[set]]$model, newdata = test_x_zero), test$rt, form = "traditional")
...
# Penalized Linear Model Variable Importance

ELN_variable_importance <- function(pooled_panel, timeSlices, model_fit_stats) {
  variable_importance_list <- rep(list(0), 3)
  ...
  for (set in 1:3) {
    ...
    original_R2 <- R2(predict(model_fit_stats[[set]]$model, newdata = test_x, lambda = model_fit_stats[[set]]$hyperparameters$lambda), test$rt, form = "traditional")
    
    new_R2 <- R2(predict(model_fit_stats[[set]]$model, newdata = test_x_zero, lambda = model_fit_stats[[set]]$hyperparameters$lambda), test$rt, form = "traditional")
...
RF_variable_importance <- function(pooled_panel, timeSlices, model_fit_stats) {
  variable_importance_list <- rep(list(0), 3)
  ...
  for (set in 1:3) {
    ...
    original_R2 <- R2(predict(model_fit_stats[[set]]$model, newdata = test_x)$predicted, test$rt, form = "traditional")
    
    new_R2 <- R2(predict(model_fit_stats[[set]]$model, newdata = test_x_zero)$predicted, test$rt, form = "traditional")
...
# Neural Network Variable Importance TODO, NOT YET WORKING

NNet_variable_importance <- function(pooled_panel, timeSlices, model_fit_stats) {
  variable_importance_list <- rep(list(0), 3)
  ...
  for (set in 1:3) {
    ...
    variable_importance <- data.frame(variable = colnames(test_x)[1], importance = (
      R2(predict(model_fit_stats[[set]]$model, newdata = test_x)$predicted, test$rt, form = "traditional") -
        R2(predict(model_fit_stats[[set]]$model, newdata = test_x_zero)$predicted, test$rt, form = "traditional")
      )
    )
    ...
    for (i in 2:ncol(test_x)) {
      ...
      variable_importance <- rbind(variable_importance, 
                                   data.frame(variable = colnames(test_x)[i], 
                                              importance = (R2(predict(model_fit_stats[[set]]$model, newdata = test_x)$predicted, test$rt, form = "traditional") -
                                                                       R2(predict(model_fit_stats[[set]]$model, newdata = test_x_zero)$predicted, test$rt, form = "traditional")
                                                            )
                                              )
                                   )
