...
# Observations regards to data

# g3 Non linear setting
# ELN + MSE does worse than ELN + MAE
# Both seem to do much better as training sample increases
```
...
NNet_fit_stats <- function(pooled_panel, timeSlices, hidden_layers, loss_function, batch_size, patience) {
  ...
  for (set in 1:3) {
    ...
    #Validation
    valid_predict <- neural_network %>% predict(as.matrix(validation_x))
...
NNet_1_mse_stats <- NNet_fit_stats(pooled_panel, timeSlices, 1, "mse", batch_size = 512, 5)
NNet_1_mae_stats <- NNet_fit_stats(pooled_panel, timeSlices, 1, "mae", batch_size = 512, 5)
