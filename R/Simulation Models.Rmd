...
LSTM_fit_stats <- function(pooled_panel, timeSlices, loss_function, batch_size, patience) {
  ...
  for (set in 1:3) {
    ...
    LSTM_stats[[set]]$loss_stats$train_RMSE <- rmse(Y_array_train %>% as.vector(), )
    ...
    #Forecast residuals
    LSTM_stats[[set]]$forecast_resids <- nnet_ave_forecast_resids(nnet_model = neural_network, test = test)
    
    #Variable Importance
    LSTM_stats[[set]]$variable_importance <- LSTM_variable_importance(X_array_test, test_x, lstm_model)
...
fforma_data_matrix <- function(panel) {
  data <- foreach(i = 1:length(unique(panel$stock)), .combine = "rbind") %do% {
  stock_id <- unique(panel$stock)
...
fforma_fit_individual <- function(panel, timeSlices, set) {
  ...
  umap_model <- umap(pooled_panel_train %>% select(-rt, -stock, -time), ret_model = TRUE)
  
  pooled_panel_train_umap <- umap_model$embedding
  
  pooled_panel_validation_umap <- umap_transform(pooled_panel_validation %>% select(-rt, -stock, -time), model = umap_model)
  
  pooled_panel_test_umap <- umap_transform(pooled_panel_test %>% select(-rt, -stock, -time), model = umap_model)
  ...
  dataset_list <- foreach(i = 1:length(unique(pooled_panel$stock))) %dopar% {
    ...
    ugarch_forecast_umap <- ugarchforecast(ugarch_spec_fit, n.ahead = 1, n.roll = out.sample_length - 1,
                                           external.forecasts = rbind(time_series_validation_umap, time_series_validation_umap) %>% list())
...
set_1[[1]]$validation_error_rsquare
```
...
## Function that returns a dataset list, each element containing:
...
```{r}
##
summary_performance <- function(predictions, dataset) {
