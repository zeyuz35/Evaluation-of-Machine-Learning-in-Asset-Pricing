...
pred <- predict(bst, newdata = xgboost::xgb.DMatrix(as.matrix(validation_list$data)), outputmargin = TRUE, reshape=TRUE)
pred <- t(apply(pred, 1, softmax_transform))

## Generate final forecasts for a set, given the meta-learner weights
## i denotes time series no., j denotes method
...
## Set up a Bayesian Optimization for the hyperparameters, using the validation loss for monitoring
## BayesianOptimization requires:
## FUN, a function to be optimized which returns a named list containing:
## Score - the metrics to be MAXIMIZED
## Pred - the actual predictions

bayes_xgb <- function(max_depth, eta, subsample, colsample_bytree, nrounds) {
  param <- list(max_depth = max_depth, eta = eta, nthread = 12,
                objective = error_softmax_obj,
                num_class = ncol(errors),
                subsample = subsample,
...
hyperparameter_search <- function(pooled_panel, timeSlices) {
