...
# Change this to 10 + 3 + 3 initially, maybe more stable procedure

customTimeSlices <- function(start, end, initialWindow, horizon, validation_size, test_size) {
  total <- c(start:end)
  offset <- start - 1
...
timeSlices <- customTimeSlices(start = 2, end = 181, initialWindow = 120, horizon = 12, test_size = 36)
...
# Foreach implementation, commented out for reference
# NNet_variable_importance <- function(test, nnet_model) {
#   
#   test_x <- test[4:ncol(test)]
#   
#   variable_importance_df <- foreach(i = (1:ncol(test_x)), .combine = "rbind", .packages = c("keras", "tensorflow")) %dopar% {
#     test_x_zero <- test_x
#     test_x_zero[, i] <- 0
#     
#     original_R2 <- R2(predict(nnet_model, as.matrix(test_x)), test$rt, form = "traditional")
...
# NNet_1_mse_stats <- NNet_fit_stats(pooled_panel, timeSlices, 1, "mse", batch_size = 16, 10)
