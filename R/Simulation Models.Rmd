...
# Observations
# These don't do too badly on linear data
# They identify significant factors correctly, as long as a very small significance value is used

# Fail completely on extremely non-linear data
# Thinks all factors are important
# In sample r squared values are somewhat high
# Absolutely atrocious validation and test r squared (both effectively test sets here)
```
...
ELN_fit_stats <- function(alpha_grid, nlamb, timeSlices, pooled_panel, loss_function) {
  ...
  for (set in 1:3) {
    ...
    #Hyperparameters
    ELN_stats[[set]]$hyperparams <- best_model_params
...
R2(NN2_mae_predictions, test$rt, form = "traditional")
R2(NN2_mse_predictions, test$rt, form = "traditional")
```
...
neural_network_3_mae <- build_NN3("mae")
...
neural_network_4_mae <- build_NN4("mae")
...
```{r}
# Random Forest Case (because the predict method for rfsrc is a little different to the others)

test <- pooled_panel %>%
  filter(time %in% timeSlices[[3]]$test)
...
variable_importance <- data.frame(variable = colnames(test_x)[1], importance = (
  R2(predict(RF_MSE[[3]]$model, newdata = test_x)$predicted, test$rt, form = "traditional") -
    R2(predict(RF_MSE[[3]]$model, newdata = test_x_zero)$predicted, test$rt, form = "traditional")
  )
)
...
for (i in 2:ncol(test_x)) {
  ...
  variable_importance <- rbind(variable_importance, 
                               data.frame(variable = colnames(test_x)[i], 
                                          importance = (R2(predict(RF_MSE[[3]]$model, newdata = test_x)$predicted, test$rt, form = "traditional") -
                                                                   R2(predict(RF_MSE[[3]]$model, newdata = test_x_zero)$predicted, test$rt, form = "traditional")
                                                        )
                                          )
                               )
