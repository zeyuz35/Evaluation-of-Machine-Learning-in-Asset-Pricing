...
\item Contrasts with the strict view that risk factors should be variables that have unpredictable variation through time, and be able to explain cross sectional returns independently
\end{itemize}
...
\item Early Example - Dividend Ratio
\item Can be \textit{persistent} (\cite{goetzmann_testing_1993}, \cite{ang_stock_2006})

\item Dividend Ratios are correlated with lagged dependent variables on the right hand side of the regression equation. 

\item Violates the assumptions of independent regressors required for OLS: t stats which are biased upwards and increase with time horizon due to autocorrelated errors

\item Corrections to t statistics using the GMM and NW errors are also shown to be biased, \citep{goetzmann_testing_1993}
\end{itemize}
\end{frame}
...
\frametitle{Background}
\begin{itemize}
\item Traditionally, dividend ratios have been included because they have shown good in sample performance, particularly in the 1990s \citep{goyal_predicting_2003}

\item Proved to be not robust and have poor out of sample performance, especially since 2000s (\cite{goyal_predicting_2003}, \cite{lettau_consumption_2001}, \cite{schwert_anomalies_2003})
...
\begin{itemize}
	...
	\item \cite{gu_empirical_2018}'s models do not have entirely consistent importance metrics - only their tree based methods recognise dividend yield as important
\end{itemize}
\end{frame}
...
\frametitle{Motivations}
\begin{itemize}
\item This paper will be the first to explore how machine learning performs in environments similar to financial data, with particular focus on how they deal with the challenges in financial data, acting as an extension to the simulation work of \cite{gu_empirical_2018}. 
...
\begin{itemize}
	...
	\item $e_{1t}$ and $e_{2t}$ will instead represent the average forecast errors for each model
\end{itemize}
\end{frame}
