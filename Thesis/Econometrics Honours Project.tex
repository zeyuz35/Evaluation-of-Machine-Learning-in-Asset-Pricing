...
\title{Empirical Asset Pricing via Machine Learning}
...
The ambiguity surrounding the true functional forms of how the predictors interact and influence the risk premia is another problem which machine learning is well suited to dealing with. Machine learning methods are highly flexible, able to capture highly non-linear patterns, but with the availability of parameter penalization and validation sets can still avoid overfitting and discovering non-existent patterns.

\todo{probably focus more on simulation aspect, pending}
