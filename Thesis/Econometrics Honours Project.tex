...
For example, valuation ratios such as dividend yield remain popular, despite being contingent on being stationary, for which there was mixed evidence even when initially introduced, \citep{vuolteenaho_understanding_1999}, and were later shown to seemingly break down, (\cite{lettau_consumption_2001}, \cite{schwert_anomalies_2003} and others). This has not stopped the literature searching for more factors: many quantitative trading firms were using 81 factor models as the norm \citep{hsu_finding_2014} by 2014, and as of the time of writing well over 400 as documented by \cite{harvey_census_nodate}, despite 

The aforementioned problems has posed immense problems, with much of the literature developing models to try to deal with this. To this end, machine learning algorithms have emerged and been recognised as well suited: 

More recently, machine learning has been recognised to be well suited to the problem of predicting stock returns: authors such as \cite{hsu_finding_2014} and \cite{gu_empirical_2018} demonstrate that machine learning outperforms significantly. 

However, though machine learning shows great promise in solving some problems in empirical finance such as factor selection (sources), there still has been no work done as to how well machine learning performs in lieu of other problems such as persistent regressors, cross sectional correlation, etc.

Prior work been done by \cite{gu_empirical_2018}, however; only basic simulation designs which did not fully explore the characteristic problems present in financial datasets were used. 
...
This model can capture non-linearities only if the predictor set \(z^*_{i, t}\) contains specified non-linear transformations or interaction terms. It is quite common to consider at least second order terms and two way interactions. 
...
The Huber Loss metric offers a combination of mean squared error and mean absolute error, with errors smaller than the threshold $\xi$ being mean squared error and errors larger than the threshold $\xi$ being absolute error. This allows for the model fit to be less sensitive to outliers which are quite common in financial data. 

\begin{figure}
	\begin{center}
		\begin{tikzpicture}
		\begin{axis}[ xlabel={$x$}, ylabel={$y$}, axis lines=middle, samples=41, grid, thick, domain=-4:4]
		\addplot+[no marks] {x^2};
		\addlegendentry{Mean Squared Error}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Illustration of MSE and Huber Loss when $\xi = 1$}
\end{figure}

\rhs{cite Huber loss}
\rhs{cite something about outliers in financial data}
\rhs{diagram representing huber loss would be great}
...
was used for all hidden layers owing to its high computational speed, and hence popularity within recent literature  (see \cite{lecun_deep_2015} and \cite{ramachandran_searching_2017}). Other potential choices for activation functions such as sigmoid, softmax, tanh etc. were not used due to computational cost.
...
(1)\; & g_1 \left(z_{i, t}\right)=\left(c_{i 1, t}, c_{i 2, t}, c_{i 3, t} \times x_{t}'\right) \theta_{0};
	\quad \text { where } \theta_{0} = (0.02, 0.02, 0.02)^{\prime} \\
(2)\; & g_2 \left(z_{i, t}\right)=\left(c_{i 1, t}^{2}, c_{i 1, t} \times c_{i 2, t}, \operatorname{sgn}\left(c_{i 3, t} \times  x_{t}'\right)\right) \theta_{0}; 
	\quad \text { where } \; \theta_{0} = (0.04, 0.035, 0.01)^{\prime} \\
(3)\; & g_3 \left(z_{i, t}\right) = \left(1[c_{i3,t}>0],c_{i 2, t}^{3}, c_{i 1, t} \times c_{i 2, t}\times 1[c_{i3,t}>0], \text{logit}\left({c}_{i 3, t} \right)\right) \theta_{0};
	\quad \text { where } \theta_{0} = (0.04, 0.035, 0.01, 0.01)^{\prime}  \\
...
\cite{breiman_classification_2017}
