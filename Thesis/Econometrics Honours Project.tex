...
Both the rolling window and recursive schemes are very computationally intensive. Therefore, a hybrid of the rolling and recursive schemes was considered: the training set is increased by one year with each refit, the validation set remains one year in length but moves forward by one year, and forecasts are only made using that model for the subsequent year. Cross validation was not done to maintain the temporal ordering of the data.
...
\begin{figure}
	\begin{center}
		\begin{neuralnetwork}
			...
			height=5, toprow=false, nodesize=20pt, style={},
			title={}, titlestyle={}]
			\newcommand{\nodetextclear}[2]{}
		    %use \ifnum to get different labels, such as x_n on the last neuron
			\newcommand{\nodetextx}[2]{\ifnum #2=5 $x_n^{(0)}$ \else $x_#2^{(0)}$ \fi}
			\newcommand{\nodetexty}[2]{$y_#2$}
			%Hidden layer textcommands
				%32 neurons
				\newcommand{\nodetextxa}[2]{\ifnum #2=5 $x_{32}^{(1)}$ \else $x_#2^{(1)}$ \fi}
				%16 neurons
				\newcommand{\nodetextxb}[2]{\ifnum #2=5 $x_{16}^{(2)}$ \else $x_#2^{(2)}$ \fi}
				%8 neurons
				\newcommand{\nodetextxc}[2]{\ifnum #2=5 $x_{8}^{(3)}$ \else $x_#2^{(3)}$ \fi}
				\newcommand{\nodetextxd}[2]{$x_#2^{(4)}$}
				\newcommand{\nodetextxe}[2]{$x_#2^{(5)}$}
			%Input Layer
			\inputlayer[count=5, bias=false, exclude = {4}, title=Input Layer, text=\nodetextx]
			%Hidden Layer 1
			\hiddenlayer[count=5, bias=false, exclude = {4}, title=Hidden Layer 1, text=\nodetextxa] 
				\linklayers[not from = {4}, not to = {4}]
			%Hidden Layer 2
			\hiddenlayer[count=5, bias=false, exclude = {4}, title=Hidden Layer 2, text=\nodetextxb] 
				\linklayers[not from = {4}, not to = {4}]
			%Hidden Layer 3
