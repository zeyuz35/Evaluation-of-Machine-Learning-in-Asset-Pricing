...
\begin{abstract}
We evaluate the application of machine learning algorithms in empirical asset pricing, specifically with regards to out of sample prediction as measured by mean squared error and mean absolute error, and factor selection evaluated using an out of sample variable importance metric. We perform a comparative analysis of some common methods in the machine learning literature: linear models, penalized linear models, random forests and neural networks. We focus on exploring how well machine learning methods can deal with characteristics common in financial data, such as low signal-to-noise ratio, cross sectional correlation, persistence, non-linear data generating processes, and stochastic volatility through a simulation study incorporating these phenomena. We identify that the best performing models in terms of prediction are penalized linear models and random forests, and the best model for factor selection are penalized linear models. We find that this result is mirrored even in empirical settings through an empirical study.
\end{abstract}
...
\rhs{the most important part of the paper, and the only part which most people will read}
\rhs{should therefore be somewhat self contained}
...
\rhs{remember to sprinkle some of the conclusion here!}

Minimizing median quantile loss yields better predictive performance.

Penalized linear models and to a lesser extent random forests have the best prediction performance.

Penalized linear models appear highly suitable for identifying true causal regressors.

More complex models such as random forests and neural networks did not outperform penalized linear models. 

Neural networks failed to outperform other machine learning models by a significant degree and are unstable. 
...
\rhs{What have you learnt about the use of this as it applies to real data? Is this similar to Gu et al and others? Draw conclusions with what has been done!}
...
\rhs{INSERT A MOTIVATION FOR THIS EMPIRICAL STUDY}
...
Across all models except for linear models, we find that minimizing quantile loss yields better prediction performance, a result which is consistent in both the simulation and empirical study.
...
Table \ref{ind_factors} contains the details of the individual factor dataset, used by \cite{gu_empirical_2018} and available from Gu's website. The table includes the factor name, an explanation of the factor, a citation to the original sources, the database source and updating frequency. Note that not all factors in this set were used, due to missing data issues the ones omitted are marked with a footnote.

Table \ref{macro_factors} contains the details of the macroeconomic factors used in the empirical study, sourced and constructed according to \cite{welch_comprehensive_2008} and available from their website.
