...
Several choices exist for the choice of penalty function \( \phi(\theta;.) \). We restrict our scope to the popular "elastic net"\footnote{We will henceforth use the terms penalized linear models and elastic net models interchangeably} penalty \citep{zou_regularization_2005}:
...
The hyperparameters $\lambda$ and $\rho$ are both tuned using the validation sample (see Section \ref{eln_comp} in Appendix). 
...
\label{empirical_study_appendix}

\begin{figure}[!htb]
	\includegraphics[max size = {\textwidth}{0.5\textheight}]{empirical_test_mse.pdf}
	\caption{Empirical Test MSE averaged across all samples}
\end{figure}

\begin{figure}[!htb]
	\includegraphics[max size = {\textwidth}{0.5\textheight}]{empirical_test_mae.pdf}
	\caption{Empirical Test MAE averaged across all samples}
\end{figure}

\begin{figure}[!htb]
	\includegraphics[max size = {\textwidth}{0.5\textheight}]{empirical_test_rsquare.pdf}
	\caption{Empirical Test Predictive R-Squared averaged across all samples}
\end{figure}
