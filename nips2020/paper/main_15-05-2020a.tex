...
\begin{enumerate}
	\item the model used to make predictions; 
...
The choice of loss function used to fit the ML methods is instrumental in the methods ultimate performance. Herein, we consider two separate loss functions: Mean Absolute Error (MAE) and Mean Squared Error (MSE). 
Mean absolute error (MAE) is the average magnitude of errors:
\begin{equation}
...
\end{align}places higher weight on large errors. Models that minimize this metric are therefore more sensitive to outliers. 
